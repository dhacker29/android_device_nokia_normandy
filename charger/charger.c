...
struct charger {
    ...
    /*qiyanqing add*/
    gr_surface surf_charger_init;//enter charging image
    gr_surface surf_charger_full;//charger full image
    /*qiyanqing end*/
...
int get_brightness()
{
    ...
    if (brightnessfd < 0) {
        brightnessfd = open("/sys/class/leds/lcd-backlight/brightness", O_RDWR);
        if (brightnessfd < 0) {
            return 0;
        }
    }

    lseek(brightnessfd, 0, SEEK_SET);
    rc = read(brightnessfd, buffer, sizeof(buffer));
    buffer[rc % 80] = '\0';
...
void set_brightness(int brightness)
{
    ...
    if (get_brightness() == 0 && brightness > 0){
        /*The param of "autosuspend_disable function" is change, so set default value*/
        autosuspend_disable();
    }
    ...
    write(brightnessfd, buffer, strlen(buffer));
...
void open_red_led()
{
    write_file("/sys/class/leds/red/brightness", "1", 1);
}
void close_blue_led()
{
...
void open_blue_led()
{
    write_file("/sys/class/leds/blue/brightness", "1", 1);
}
...
void open_green_led()
{
    write_file("/sys/class/leds/green/brightness", "1", 1);
}
...
static int64_t Screen_timeout_base;
...
int is_time_out()
{
    if (curr_time_ms() - Screen_timeout_base > SCREEN_TIMEOUT){
        return 1;
    }else{
        return 0;
    }
...
void reset_time_out()
{
    Screen_timeout_base = curr_time_ms();
}
...
/*=====================================
FUNCTION    draw_charger_init

DESCRIPTION
  when enter power off charging, display init image first.

PARAMS:
  charger struct, attentive "surf_charger_init" ,means init image resource

RETURN VALUE
  void
=====================================*/
static void draw_charger_init(struct charger *charger)
{
    int y;
    if (charger->surf_charger_init) 
    {
        draw_surface_centered(charger, charger->surf_charger_init);
        gr_flip();
        /*sleep 2s, for stay at init image enough*/
        usleep(2000000);
    }
}
...
/*=====================================
FUNCTION    draw_charger_full

DESCRIPTION
  when charge completed, display full charge image always
...
RETURN VALUE
  void
=====================================*/
static void draw_charger_full(struct charger *charger)
{
    int y;
    if (charger->surf_charger_full) 
    {
        draw_surface_centered(charger, charger->surf_charger_full);
    }
}
/*qiyanqing end*/
...
static void redraw_screen(struct charger *charger)
{
    ...
    else if(batt_anim->capacity == 100)//draw full image when battery capacity is full 
        draw_charger_full(charger);
    else
...
static void update_screen_state(struct charger *charger, int64_t now)
{
    ...
    /* FIXME: remove this call which conflicts with continuous splash */
...
{
    process_key(charger, KEY_END, now);
    ...
    while (true) {
        ...
        if ((!is_usb_exist()) && (!is_ac_exist()) && (!is_unknown_exist())){
            ...
            android_reboot(ANDROID_RB_POWEROFF,
                (ANDROID_RB_FLAG_NO_SYNC|ANDROID_RB_FLAG_NO_REMOUNT_RO), 0);
        }
        ...
        if (get_brightness() != 0 && is_time_out()){
            set_brightness(0);
        }
...
int main(int argc, char **argv)
{
    ...
    /*qiyanqing add*/
    /*create init charge image surface*/
    ret = res_create_surface("charger/battery_init", &charger->surf_charger_init);
    if (ret < 0) {
        LOGE("Cannot load battery_init image\n");
        charger->surf_charger_init = NULL;
    }
    /*create full charge image surface*/
    ret = res_create_surface("charger/battery_full", &charger->surf_charger_full);
    if (ret < 0) {
        LOGE("Cannot load battery_full image\n");
        charger->surf_charger_full = NULL;
    }
    /*qiyanqing end*/
    ...
    reset_time_out();
    /*qiyanqing add*/
    /*when enter power off charging first, draw init charge image*/
    draw_charger_init(charger);
    /*qiyanqing end*/
    event_loop(charger);
